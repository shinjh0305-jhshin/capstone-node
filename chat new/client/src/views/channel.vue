<template>
    <section class="channel" @click="Channelrotate=false" :style="channel">
            <section class="overlay" v-if="overlay" @click.self="overlay=false">
                   <div class="new-channel" v-if="newChannel">
                       <p>NEW CHANNEL <span class="error">{{error}}</span></p>
                       <input type="text" placeholder="name" v-model="channelDetails.name" @focus="created=false">
                       <textarea placeholder="description" cols="30" rows="10" v-model="channelDetails.description"></textarea>
                       <button class="join" @click="addNewChannel(channelDetails)" :disabled="creating || created">
                          <span v-if="!creating && !created">Create</span>
                          <img src="//s.svgbox.net/loaders.svg?fill=ffffff#oval" v-if="creating">
                          <img src="//s.svgbox.net/materialui.svg?fill=ffffff#check_circle_outline"  v-if="created">
                       </button>
                   </div>
                   <div class="new-channel" v-if="joinChannel">
                       <p>JOIN A CHANNEL</p>
                       <input type="text" placeholder="Input channel name" v-model="findChannelbyName">
                       <p>{{searchError}}</p>
                       <button class="join" @click="findChannel(findChannelbyName)" :disabled="finding"><span v-if="!finding">Search</span><img src="//s.svgbox.net/loaders.svg?fill=ffffff#oval" v-if="finding"></button>
                       <div class="foundChannel" v-if="foundChannel">
                            <img src="//s.svgbox.net/materialui.svg?fill=fffff5#close" alt="close" class="close" @click="foundChannel=false">
                            <img src="@/assets/channel-avi.svg" alt="" class="avi">
                            <p class="title">{{foundChannel.name}}</p>
                            <p>{{foundChannel.description}}</p>
                            <p>{{joiningError}}</p>
                            <button class="join" :disabled="joining || joined" @click="joinNewChannel(foundChannel)">
                                <span v-if="!joining && !joined">Join</span>
                                <img src="//s.svgbox.net/loaders.svg?fill=ffffff#oval" v-if="joining">
                                <img src="//s.svgbox.net/materialui.svg?fill=ffffff#check_circle_outline" v-if="joined">
                            </button>
                       </div>
                   </div>
            </section>
            <section class="left" :class="{'showSectionLeft':show}">
                 <div class="joinOrCreate" :class="{'shrink':joinOrCreate}">
                      <img src="//s.svgbox.net/materialui.svg?fill=fffff5#close" class="close" @click="joinOrCreate=true">
                      <p class="joinOrCreate">Create or Join a channel</p>
                      <p class="desc">Channel is where you and your friends hang out. Join one and start having fun</p>
                      <div class="create mt-5"  @click.stop="showOption('create')">
                           <div>
                                <img src="https://discord.com/assets/79516499036b21acd5f56ccba0635c38.svg" alt="discord">
                      ...
                      <div class="create"  @click.stop="showOption('join')">
                           <div>
                                 <img src="https://discord.com/assets/6670414aa93c6a4b3b5c7f57ead40533.svg" alt="discord">
                                 <p class="desc">Join a channel</p>
                           </div>
                           <i class="fa fa-chevron-right"></i>
                      </div>
                </div>
                <channels :channels='channels' @leave-channel="leaveChannel"  @add-channel="joinOrCreate=false" @channel-click="showChannels=false" v-if="showChannels" />
                <members  :channel="channel" @icon-click="showChannels=true" v-if="!showChannels"  />
                <div class="user">
                       ...
                       <i class="fa fa-chevron-right" :class="{'rotate':Channelrotate}" @click.stop="Channelrotate=!Channelrotate"></i>
                </div>
            </section>
            <section class="right">
                  ...
                  <div class="channel-head">
                         <p> <i class="fa fa-bars toggle" @click="show=true"></i> {{channel.name | capitalize}}</p>
                         <i class="fa fa-times cancel" @click="show=false" v-if="show" ></i>
                  </div>
                   <p class="disconnected" v-if="disconnected">disconnected</p>
                  <div class="body-right-container">
                      <div class="body-right">
                            <div class="mssg" v-for="(message,index) in channel.messages" :key="index">
                                  <div v-if="showDivider(index)" class="divider">
                                        <div></div>
                                        <p>{{message.date | dividerDate}}</p>
                                        <div></div>
                                    </div>
                                  <p class="notification" v-if="message.type==='notification'">{{message.content}}</p>
                                  <message v-if="message.type!=='notification'" :message="message" :yours="(currentUser._id===message.owner.id._id || currentUser.username===message.owner.name)" />
                            </div>
                      </div>
                  </div>
                  <div class="send">
                        <input type="text"  placeholder="Type a message here" v-model="message.content" @keyup.enter="sendMessage(channel)">
                        <button @click.stop.prevent="sendMessage(channel)" :disabled='message.content=="" || channel.name===undefined'>
  ...
  import avatar1 from '@/assets/sogang.png';
  ...
  export default {
      ...
      data(){
        return{
              ...
              joined:false,
              finding:false,
              socket:io("https://whispering-everglades-42925.herokuapp.com"),
              child:100,
              parent:30,
        }
      },
      ...
      methods:{
          ...
          findChannel(ChannelName){
              ...
              this.joiningError='';
              this.joined=false;
              this.$http.get('https://whispering-everglades-42925.herokuapp.com/channels/name/'+ChannelName).then(res=>{
                 this.finding=false;
                 if(res.data===null){
          ...
          joinNewChannel(channel){
                 this.joined=false;
                 this.joining=true;
                 this.$http.post('https://whispering-everglades-42925.herokuapp.com/channels/'+channel._id).then(res=>{
                     this.joining=false;
                     if(res.data.isHere){
          ...
          addNewChannel(channelDetails){
             this.creating=true;
             this.error=''
             this.$http.post('https://whispering-everglades-42925.herokuapp.com/channels',channelDetails).then(res=>{
                  this.creating=false;
          ...
          send(message,channel){
               ...
               this.$http.post('https://whispering-everglades-42925.herokuapp.com/channels/'+channel._id+'/messages',message).then(res=>{
          ...
          leaveChannel(channel,index){
               let channelCopy = JSON.parse(JSON.stringify(channel));
               this.$http.delete('https://whispering-everglades-42925.herokuapp.com/channels/'+channel._id).then(res=>{
      ...
      mounted(){
           ...
           this.socket.on('messageReceived',(channel)=>{
                ...
                let diff=this.child.clientHeight-(this.parent.clientHeight + this.parent.scrollTop)
                if(diff<10){
                  this.$nextTick(() => {
                  this.parent.scrollTop=this.updatedScrollTop();
                })
           ...
           this.$http.get('https://whispering-everglades-42925.herokuapp.com/users/currentUser').then(res=>{
                if(res.data.isLoggedIn===false){
                    ...
                    this.$http.get('https://whispering-everglades-42925.herokuapp.com/users/currentUser/channels').then(res=>{
  ...
  <style>
  i.fa-paper-plane,div.user i.fa-chevron-right,div.new-channel button,i.cancel{
    cursor: pointer;
  }
  section.channel{
    position: relative;
  ...
  div.new-channel textarea, div.new-channel input, div.new-channel p{
     margin: 0;
     margin-bottom: 0.7rem;
     width: 100%;
  }
  div.new-channel textarea, div.new-channel input{
    display: block;
    padding: 0.8rem;
  ...
  div.new-channel textarea:focus, div.new-channel input:focus{
