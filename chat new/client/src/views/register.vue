            ...
            <div class="form-container">
                 ...
                 <div class="form" :style="{backgroundImage:bg1}">
                     <div class="input-field" :class="{'focus':focusUser}">
                         ...
                         <input type="text" placeholder="Username" v-model="user.username" @focus="focus('user')">
                     </div>
                     <div class="input-field" :class="{'focus':focusPassword}">
                         <i class="fa fa-lock"></i>
                         <input :type="password" placeholder="Password" v-model="user.password" @focus="focus('password')">
                         <i class="fa fa-eye eye" :class="{'fa-eye-slash':eye}"  @click="togglePasswordField"></i>
                     </div>
                     <p class="error">{{error}}</p>
                     <div class="sign-in">
                          <p class="option">Already have an account? <router-link to="/login">Sign in</router-link></p>
                          <button @click="register" :disabled="disabled"><span v-if="!disabled">Continue</span><img src="//s.svgbox.net/loaders.svg?fill=ffffff#oval" v-if="disabled"></button>
                     </div>
      ...
      methods:{
          register(){
              ...
              if(this.validateForm(self.user)){
                   this.disabled=true;
                   this.$http.post('https://whispering-everglades-42925.herokuapp.com/auth/register',self.user).then((response)=>{
                   this.disabled=false;
