...
export default {
  ...
  props: ["messsage", "yours"],
  computed: {
    avi() {
      if (this.message.owner.id.avi == undefined) return this.avatar;
      else return this.message.owner.id.avi;
    },
  },
  filters: {
    messageDate(value) {
      const date = daysjs(value);
      ...
      if (now.isSame(date, "day")) {
        ...
        now.diff(date, "day", true) > 0 &&
        now.diff(date, "day", true) < 1
      ) {
