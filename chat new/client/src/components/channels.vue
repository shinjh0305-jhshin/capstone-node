...
<template>
  <div class="channel-container">
    <div class="channels-header">
      <p class="add-channel" @click="$emit('add-channel')">
      ...
      >
        <router-link :to="{ name: 'channel', params: { id: channel._id } }">
          <div class="channel-name">
            <span :class="{ picked: $route.params.id === channel._id }"
              >CH</span
            >
            <p>{{ channel.name }}</p>
          </div>
        ...
        <i
          ...
          v-if="$route.params.id === channel._id"
  ...
  computed: {
    computedChannels() {
      return this.channels.filter((channel) => {
        return channel.name.includes(this.channelSearch);
      });
    },
  },
  methods: {
    confirm(index) {
      return index === this.confirmIndex;
    },
